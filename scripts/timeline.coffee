...
module.exports = (robot) ->
  ...
  robot.hear /.+/, (msg) ->
    ...
    room = robot.adapter.client.rtm.dataStore.getChannelGroupOrDMById(room_id).name

    # ユーザ名_channelの部屋だけウォッチ対象
    if room == "times_timeline"
      # 何もしない
    else if room.match(/^times_.+/)
      # 展開可能なURLを作成し、タイムライン表示用の部屋に投稿する
      # roomの指定で、 投稿するchannelを指定
      # 第二引数でslackのパーマネントURLを構築
      robot.send {room: "#times_timeline"}, "#{slack_url}/archives/#{room_id}/p#{id}"
